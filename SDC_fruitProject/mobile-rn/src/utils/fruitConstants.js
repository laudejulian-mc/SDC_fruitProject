...
/**
 * Returns a contextual insight object based on fruit, predicted label, and confidence.
 * { verdict, advice, safetyIcon, nutritionTip }
 */
export function getConfidenceInsight(fruit, label, confidence) {
  ...
  if (isFresh) {
    if (pct >= 90) {
      return {
        verdict: `Excellent quality! This ${fruit} is very fresh and safe to eat.`,
        advice: `At ${pct}% confidence, this fruit shows strong signs of freshness. ${info.freshTip} Enjoy it raw, in salads, or as a healthy snack.`,
        safetyIcon: '✅',
        safetyLevel: 'safe',
        nutritionTip: `${info.calories} per serving. Rich in ${info.keyNutrients}. ${info.benefits}`,
        storageTip: info.storage,
      };
    } else if (pct >= 75) {
      return {
        verdict: `Good quality. This ${fruit} appears fresh and is safe to eat.`,
        advice: `At ${pct}% confidence, the fruit looks fresh with minor imperfections. ${info.freshTip} Best consumed within 1-2 days for peak quality.`,
        safetyIcon: '👍',
        safetyLevel: 'good',
        nutritionTip: `${info.calories} per serving. Rich in ${info.keyNutrients}. ${info.benefits}`,
        storageTip: info.storage,
      ...
      return {
        verdict: `Likely fresh, but inspect closely before eating.`,
        advice: `At ${pct}% confidence, there may be early signs of aging. Check for ${info.rottenSign.toLowerCase()} If the fruit looks and smells fine, it should still be safe to eat.`,
        safetyIcon: '🔍',
        safetyLevel: 'caution',
        nutritionTip: `${info.calories} per serving. ${info.keyNutrients}. Consider consuming soon for best nutrition.`,
        storageTip: `Consume soon. ${info.storage}`,
      };
    }
  } else {
    // Rotten / not fresh
    if (pct >= 90) {
      return {
        verdict: `Not safe to eat. This ${fruit} shows clear signs of spoilage.`,
        advice: `At ${pct}% confidence, this fruit is definitely ${label.toLowerCase()}. Signs: ${info.rottenSign} Do not consume — discard or compost it.`,
        safetyIcon: '🚫',
        safetyLevel: 'danger',
        nutritionTip: 'Spoiled fruit may contain harmful bacteria and mold toxins. Consuming it can cause food poisoning.',
        storageTip: 'Discard this fruit. Separate it from other fresh produce to prevent contamination.',
      };
    } else if (pct >= 75) {
      return {
        verdict: `Likely spoiled. Not recommended for consumption.`,
        advice: `At ${pct}% confidence, this fruit shows significant signs of decay. Look for: ${info.rottenSign} It is best to discard it.`,
        safetyIcon: '⚠️',
        safetyLevel: 'warning',
        nutritionTip: 'Partially spoiled fruit may still harbor harmful microorganisms even if only part looks bad.',
        storageTip: 'Discard and check nearby fruits for signs of spoilage spreading.',
      };
    } else {
      return {
        verdict: `Quality is questionable. Inspect carefully.`,
        advice: `At ${pct}% confidence, this fruit may be in early stages of spoilage. Check for: ${info.rottenSign} If only surface blemishes, you may cut away affected areas.`,
        safetyIcon: '🤔',
        safetyLevel: 'uncertain',
        nutritionTip: `If usable parts remain, nutritional value may be reduced. Original: ${info.calories}, ${info.keyNutrients}.`,
        storageTip: 'Use immediately if salvageable. Do not store further.',
      };
    }
  }
}
